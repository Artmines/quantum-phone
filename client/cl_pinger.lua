...
RegisterNUICallback("Pinger/SendPing", function(Data, Cb)
    local Success = CallbackModule.SendCallback("quantum-phone/server/pinger/do-ping-request", Data, GetEntityCoords(PlayerPedId()))
    Cb(Success)
end)

function Pinger.Render()
    exports['quantum-ui']:SendUIMessage("Phone", "RenderPingerApp", {
...
RegisterNetEvent('quantum-phone/client/pinger/receive', function(Coords, Sender, Name)
    -- Reset Data
    Pinger.Accepted = false
            ...
            Buttons = {
                {
                    ...
                    Event = "quantum-phone/client/pinger/answer-pinger",
    ...
    Citizen.SetTimeout(10000, function()
        if Pinger.Accepted == false then
            TriggerEvent('quantum-phone/client/pinger/answer-pinger', { Declined = true, Sender = Sender, Coords = Coords })
            TriggerEvent('quantum-phone/client/hide-notification', Pinger.Id)
        end
    end)
end)

RegisterNetEvent('quantum-phone/client/pinger/answer-pinger', function(Data)
    Pinger.Accepted = true
    TriggerServerEvent('quantum-phone/server/pinger/sender-message', Data.Sender, (Data.Declined and 'Ping was denied..' or 'Ping was accepted!'))
