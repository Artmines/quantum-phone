...
Citizen.CreateThread(function()
    ...
    CallbackModule.CreateCallback('quantum-phone/server/housing/get-owned-houses', function(Source, Cb)
            ...
            if Houses[1] ~= nil then
                for k, v in pairs(Houses) do
                    HouseData[v.house] = {
                        Owner = v.citizenid,
                        Name = v.house,
                        Locked = exports['quantum-housing']:IsHouseLocked(v.house),
                        Category = v.category,
                        Adres = v.label,
    ...
    CallbackModule.CreateCallback('quantum-phone/server/housing/purchase-house', function(Source, Cb, Data)
        local Player = PlayerModule.GetPlayerBySource(Source)
        ...
        if Player.Functions.RemoveMoney('Bank', Data.House.Price, "Purchased house") then
            ...
            DatabaseModule.Update('UPDATE player_houses SET citizenid = ?, owned = ?, keyholders = ? WHERE house = ?', {
                ...
                exports['quantum-housing']:RefreshHousing()
                Cb(true)
            end)
        else
            Cb(false)
    ...
    CallbackModule.CreateCallback('quantum-phone/server/housing/toggle-locks', function(Source, Cb, Data)
        local DoorState = exports['quantum-housing']:ToggleDoorLocks(Data.HouseId)
        Cb(DoorState)
    end)

    CallbackModule.CreateCallback('quantum-phone/server/housing/sell-house', function(Source, Cb, Data)
        local Player = PlayerModule.GetPlayerBySource(Source)
    ...
    CallbackModule.CreateCallback('quantum-phone/server/housing/get-keyholders', function(Source, Cb, Data)
        DatabaseModule.Execute('SELECT * FROM player_houses WHERE house = ?', {
            Data['HouseId'],
        }, function(House)
    ...
    CallbackModule.CreateCallback('quantum-phone/server/housing/add-key', function(Source, Cb, Data)
        DatabaseModule.Execute('SELECT * FROM player_houses WHERE house = ?', {
            Data['HouseId'],
        }, function(Houses)
            if Houses[1] ~= nil then
                ...
                DatabaseModule.Update('UPDATE player_houses SET keyholders = ? WHERE house = ?', {
                    ...
                    TriggerClientEvent('quantum-phone/client/notification', TPlayer.PlayerData.Source, {
                        Id = math.random(100000, 999999),
                        Title = 'Housing',
    ...
    CallbackModule.CreateCallback('quantum-phone/server/housing/remove-key', function(Source, Cb, Data)
        DatabaseModule.Execute('SELECT * FROM player_houses WHERE house = ?', {
            Data['HouseId'],
        }, function(Houses)
            if Houses[1] ~= nil then
